...
def generate_content(doc_data: bytes, api_key: str, schema: str):
    ...
    # Uses httpx to pull file from online S3 bucket, e.g. https://datasheetminer.s3.us-east-1.amazonaws.com/motor.json
    with httpx.Client() as http_client:
        response = http_client.get(
            f"https://datasheetminer.s3.us-east-1.amazonaws.com/schema/{schema}.json"
        )
        schema_str = response.text

    prompt = f"Identify the individual components along with their key specifications based on this guidance {SCHEMA_GUIDANCE} and this schema: {schema_str} in the information provided. Create a list of JSON objects with each product found.\n\n{GUARDRAILS}"
