...
| 3 | [UNITS.md](UNITS.md) | 🚧 code ✅ shipped 2026-04-28 (`a8f6162`); data backfill pending operator | 🟡 medium (½ day review) | **Linchpin.** Phases 1–6 done — `ValueUnit`/`MinMaxUnit` are real BaseModels carrying `{value, unit}` end-to-end, compact-string layer deleted. Phase 5's `cli/migrate_units_to_dict.py` script exists; running it against dev → staging → prod (with `--dry-run` first) is the operator's call. |
