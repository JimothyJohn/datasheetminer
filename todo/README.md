...
| 4 | [INTEGRATION.md](INTEGRATION.md) | ✅ phases A+B + build-complete affordances all shipped 2026-04-30 (Copy BOM, "looks complete" badge, ChainReviewModal). Stretch "Save build as preset" parked. | 🟢 small | Motion-system builder — drive → motor → gearhead. Only remaining work is the deferred "Save build as preset" stretch (one-project users won't miss it) and Phase D spec-first wizard (large, future). |
...
| 6 | [FRONTEND_TESTING.md](FRONTEND_TESTING.md) | ✅ all 8 phases shipped 2026-04-30 (per-key persistence + L6 fix, AppContext setter contract, ProductList type-switch reset + L1 fix, header toggles, FilterChip unit propagation, BuildTray + compat junctions, ErrorBoundary, route smoke render); suite 22 files / 349 tests | 🟢 small (half-day, 8 phases) | Locked down the L1–L12 spillover bestiary. Optional follow-ups: full DOM-level ProductList type-switch test, vitest coverage gate in CI, Playwright visual regression layer. |
