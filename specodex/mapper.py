...
def results_to_manufacturers(results: List[Dict[str, Any]]) -> List[Manufacturer]:
    ...
    for res in results:
        manufacturers.append(
            Manufacturer(name=res.get("title", "Unknown"), website=res.get("link"))
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Datasheetminer Mapper - Find Manufacturers using Google Search"
    )
    parser.add_argument("query", help="What to search for (e.g. 'industrial motors')")
    parser.add_argument(
        "--limit", type=int, default=10, help="Max number of results to retrieve"
    )
    parser.add_argument(
        "--api-key",
        help="Google Search API Key (defaults to GOOGLE_SEARCH_API_KEY env var)",
    )
    ...
    if not api_key:
        print(
            "Error: detailed API key is required. Set GOOGLE_SEARCH_API_KEY or pass --api-key."
        )
        sys.exit(1)
