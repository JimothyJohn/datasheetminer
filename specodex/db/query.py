...
class TableInspector:
    ...
    def count_items(self) -> Dict[str, int]:
        ...
        robot_arm_count: int = sum(
            1 for item in all_items if isinstance(item, RobotArm)
        )
...
Examples:
  # Show table summary
  uv run specodex/query.py --table products --summary

  # List all items (summary view)
  uv run specodex/query.py --table products --list

  # List products with full details
  uv run specodex/query.py --table products --list --type drive --details

  # List first 5 motors
  uv run specodex/query.py --table products --list --type motor --limit 5

  # Get specific item by ID
  uv run specodex/query.py --table products --get <item-id> --type drive
